...
async function refreshRecordsFromSupabase(options = {}) {
  ...
  try {
    ...
    const [sharedRecords, sharedRepairRecords, sharedDemoRecords, sharedPricingRecords] = await Promise.all([
...
async function refreshPricingFromSupabase() {
  ...
  const sharedPricingRecords = await loadSupabasePricingRecords();
  if (!sharedPricingRecords) return;
  pricingRecords = sharedPricingRecords;
...
function renderPricingOffer() {
  ...
  if (offerPfronInput && offerPfronInput.value && pfron !== normalizePricingPrice(offerPfronInput.value)) {
...
function switchPricingView(viewName) {
  activePricingView = viewName === "offer" ? "offer" : "list";
