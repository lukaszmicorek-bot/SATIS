...
async function refreshRecordsFromSupabase(options = {}) {
  ...
  try {
    ...
    const [sharedRecords, sharedRepairRecords, sharedDemoRecords, sharedPricingRecords, sharedLoanHistory, sharedOfferHistory] = await Promise.all([
...
async function refreshPricingFromSupabase() {
  ...
  const [sharedPricingRecords, sharedLoanHistory, sharedOfferHistory] = await Promise.all([
    loadSupabasePricingRecords(),
    loadSupabasePricingLoanHistory(),
    loadSupabasePricingOfferHistory()
  ]);
  if (!sharedPricingRecords && !sharedLoanHistory && !sharedOfferHistory) return;
  if (sharedPricingRecords) pricingRecords = sharedPricingRecords;
  if (sharedLoanHistory) pricingLoanHistory = sharedLoanHistory;
  if (sharedOfferHistory) pricingOfferHistory = sharedOfferHistory;
  renderPricingRecords();
  if (activeNotebook !== "pricing") renderDeviceViews();
  setCurrentYearTitle();
}
...
function switchPricingView(viewName) {
  activePricingView = ["list", "offer", "loan"].includes(viewName) ? viewName : "list";
...
function customerDocumentInfoForRecord(record) {
  ...
  const heading = hasUncertainMatch
    ? "Dokumenty klienta (sprawdź dopasowanie - powtórzone imię i nazwisko):"
    : "Dokumenty klienta:";
  return {
